...
module.exports = class Blockchain {
  ...
  static deserializeBlock(o) {
    ...
    if (b.isGenesisBlock()) {
      ...
      b.txMerkleTree = new FixedMerkleTree(MAX_BLOCK_TRANSACTIONS);

      let allTx = o.txMerkleTree.transactions;
      let allTransactions = [];
      for(let i = 0; i < allTx.length; i++) {
        if(allTx[i] != " ") {
          allTransactions.push(allTx[i]);
        }
      }
      
      if (allTransactions) {
        for(let i = 0; i < allTransactions.length; i++) {
          let tx = new Blockchain.cfg.transactionClass(allTransactions[i]);
          b.txMerkleTree.addTx(tx);
        }
      }
    }
    return b;
